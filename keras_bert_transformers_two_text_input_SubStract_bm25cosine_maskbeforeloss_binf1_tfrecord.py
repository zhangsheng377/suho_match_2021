...
def get_dataset(file_list, epochs, batch_size, need_id=False):
    return tf.data.TFRecordDataset(file_list)         .map(partial(parse_from_single_example, need_id=need_id), multiprocessing.cpu_count()-2)         .repeat(epochs)         .shuffle(buffer_size=batch_size, reshuffle_each_iteration=True)         .batch(batch_size)         .prefetch(buffer_size=batch_size*2)
